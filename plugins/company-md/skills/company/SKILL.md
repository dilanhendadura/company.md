...
For an output that needs artifact-level traceability, also run `scripts/create-receipt.mjs` after generation. For a local file, pass `--root <repository-root>` so stored paths remain portable. For a native cloud artifact, pass `--remote-url`, `--provider`, `--revision`, and `--mime-type`; include export metadata when available and record `artifact/access` plus `artifact/revision` gates. In both cases, pass `--contract generic/v1` (or the more specific required contract), the selected profile and clearance, exact source files, client sources, generated intermediates, unresolved facts, and verification gates. When a required local artifact cannot be created, use `--expected-deliverable` and blocked verification gates instead of inventing a file or hand-writing a receipt. Keep the receipt beside the artifact or its local manifest.
